	...
	public static function inscrire($identifiant, $email, $passeUn, $passeDe) {
		if(!empty($identifiant) AND !empty($email) AND !empty($passeUn) AND !empty($passeDe)) {
			if($passeUn === $passeDe) {
				...
				$resultat = 'Le champ &quot;Saisir un Mot de Passe&quot; <br>et le champ &quot;Resaisir un Mot de Passe&quot; <br>doivent &ecirc;tre identiques, <br />veuillez recommencer l\'inscription.';
			}
		}
		else {
	...
	public static function administrateur($id, $jeton, $niveau) {
		if(empty($id) OR empty($jeton)) {
			...
			if($resultat -> rowCount() !== 1) {
				...
				if(Membre::info($id, 'activation') === '5') {
					redirection(URLSITE.'/administrateur/admin/banni.php');
				}
				return true;
			}
		}
...
class InfoSite {
	...
	public static function activationChange($id) {
		...
		redirection(URLSITE.'/administrateur/admin/activation.php');
	}
	...
	public static function listeMembre($id) {
		...
		while($donnee = $resultat -> fetch(PDO::FETCH_ASSOC)) {
			...
			if($donnee['activation'] === '5') {
				$niveau = '<div class="badge badge-pill badge-danger badge-md mr-1 mb-1">Banni</div>';
				$action = '<input type="hidden" value="'.$donnee['id'].'" name="id">
							<button type="submit" class="btn btn-icon btn-warning" name="debannir" value="D&eacute;bannir"><i class="feather icon-sunrise"></i> D&eacute;bannir</button>
							<button type="submit" class="btn btn-icon btn-danger" name="supprim" value="Supprimer"><i class="feather icon-alert-triangle"></i> Supprimer</button>';
			}
			elseif($donnee['activation'] === '0') {
				$niveau = 'Nouvel(le) inscrit(e)';
				$action = '<input type="hidden" value="'.$donnee['id'].'" name="id">
							<button type="submit" class="btn btn-icon btn-danger" name="bannir" value="Bannir"  data-toggle="modal" data-target="#inlineForm"><i class="feather icon-x"></i> Bannir</button>';

			}
			else {
				switch($donnee['niveau']) {
					case 1 :
					$niveau = '<div class="badge badge-pill badge-secondary badge-md mr-1 mb-1">Membre</div>';
					$action = '<input type="hidden" value="'.$donnee['id'].'" name="id">
								<button type="submit" class="btn btn-icon btn-danger" name="bannir" value="Bannir"  data-toggle="modal" data-target="#inlineForm"><i class="feather icon-x"></i> Bannir</button>
								<button type="submit" class="btn btn-icon btn-info" name="moderateur" value="Passer Mod&eacute;rateur"><i class="feather icon-arrow-up"></i> Passer Mod&eacute;rateur</button>';
					break;
					case 2 :
					$niveau = '<div class="badge badge-pill badge-info badge-md mr-1 mb-1">Mod&eacute;rateur</div>';
					$action = '<input type="hidden" value="'.$donnee['id'].'" name="id">
								<button type="submit" class="btn btn-icon btn-danger" name="bannir" value="Bannir"  data-toggle="modal" data-target="#inlineForm"><i class="feather icon-x"></i> Bannir</button>
								<button type="submit" class="btn btn-icon btn-secondary" name="membre" value="Repasser Membre"><i class="feather icon-arrow-down"></i> Repasser Membre</button>';
					break;
					case 3 :
					$niveau = '<div class="badge badge-pill badge-success badge-md mr-1 mb-1">Administrateur</div>';
					$action = '';
					break;
				}
			}
			$liste .= '<tr>
					<td align=""><a href="profil_membre.php?id='.$idMembre.'">'.$pseudo.'</a></td>
					<td align="">'.$niveau.'</td>
					<td align="" class="product-action"><form action="" method="post">'.$action.'</form></td>
				</tr>';
		}
		return $liste;
	}
...
###########################################################################################
// La classe caractéres
class Caractere {
    public static function coupeCourt($texte,$long,$marge){
...
class Admin {
	...
	public static function bannir($id, $messagePost) {
		...
		mail(MAIL_SITE, $sujet, $message, $headers);*/
		//				*******************				//
		redirection(URLSITE.'/administrateur/admin/listeMembre.php');
	}
	// debannir un membre
